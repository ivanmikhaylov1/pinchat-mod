...
public class PinnedMessages {
  ...
  public static MessageGroup getOrCreateDefaultGroup() {
    if (groups.isEmpty()) {
      int x = ConfigHandler.getConfig().pinnedX;
      int y = ConfigHandler.getConfig().pinnedY;
      double scale = ConfigHandler.getConfig().pinnedScale;
  ...
  public static void toggle(Text message, MessageGroup targetGroup) {
    ...
    if (existingMatch != null) {
      ...
      if (targetGroup.messages.size() >= ConfigHandler.getConfig().maxPinnedMessages) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.player != null) {
          client.player.sendMessage(Text.of("§cPinChat: Maximum number of pinned messages reached ("
              + ConfigHandler.getConfig().maxPinnedMessages + ")"), false);
        }
